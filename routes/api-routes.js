...
router.post('/', (req, res) => {
	...
	if (!postInfo.title || !postInfo.title.trim()) {
		return res
			.status(400)
			.json({ errorMessage: 'Please provide a title for the post.' })
	}
	if (!postInfo.contents || !postInfo.contents.trim()) {
		return res
			.status(400)
			.json({ errorMessage: 'Please provide content for the post.' })
	}
				...
				blogPosts
					...
					.then((comment) => {
						blogPosts
							...
							.then((thecomment) => res.status(201).json(thecomment))
					})
					.catch((err) => {
						res
							.status(500)
					...
					.then((comments) => {
						if (comments.length > 0) {
							...
							res
								...
								.json({
									message:
										'The post with the specified ID does not have comments.'
								})
						}
					})
					.catch((err) => {
...
router.put('/:id', (req, res) => {
	...
	blogPosts
		...
		.then((post) => {
			if (post.length > 0) {
				blogPosts.update(id, updatedPostContent).then((updated) => {
					blogPosts
						.findById(id)
						.then((post) => {
							res.status(200).json(post)
